...
本 phase 只负责把商户端从“白名单入口”推进到可运营的工作台：商户可以查看并推进订单状态、维护一级品类与商品销售规则、搜索用户并调整余额，以及在运行时修改店铺信息、配送费、会员等级门槛、Banner 和定制提示。它必须继续遵守前几个 phase 已锁定的 Cloud Functions 安全边界与 config-driven 业务规则，但不扩展到退款售后、多门店、营销活动引擎或更复杂的 Web 后台。
...
- 订单管理会直接连接 `orders` 集合与现有订单/支付 store，并与客户端订单详情共享状态文案。
