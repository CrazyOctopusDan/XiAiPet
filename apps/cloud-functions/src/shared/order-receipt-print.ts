...
import type { MerchantManagedOrderRecord, MerchantOrderTimelineEntry } from './order-store';
...
function getFulfillmentLine(order: OrderRecord) {
...
function getContactLines(order: OrderRecord) {
  const { fulfillment } = order.snapshot;
...
export function buildOrderReceiptLines(order: OrderRecord, isReprint: boolean): string[] {
  const lines = [
  ...
  lines.push('------------------------------');
  lines.push(`打印版本：${RECEIPT_TEMPLATE_VERSION}`);
  lines.push('\n\n');

  return lines;
}

export function encodeReceiptChunks(lines: string[]): string[] {
...
export function createReceiptPrintJob(order: OrderRecord, now: string): OrderReceiptPrintJob {
  ...
  return {
    ...
    receiptTemplateVersion: RECEIPT_TEMPLATE_VERSION,
    isReprint,
...
export function applyReceiptPrintAudit(
  order: MerchantManagedOrderRecord,
  payload: OrderReceiptPrintAuditPayload
): MerchantManagedOrderRecord {
  const nextPrintCount =
    payload.result === 'success' ? (order.receiptPrint?.printCount ?? 0) + 1 : order.receiptPrint?.printCount ?? 0;
  ...
  const timelineEntry: MerchantOrderTimelineEntry = {
    type: 'print',
